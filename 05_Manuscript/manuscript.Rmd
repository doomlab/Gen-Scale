...
Data were screened for accuracy, missing data, outliers, and multivariate parametric assumptions. The undergraduate data included *n* = `r nrow(sona)` participants. We first screened each participant for missing data and excluded *n* =  `r nrow(sona) - nrow(sona_replaced)` participants for missing more than 5% of their data [TABACHNICK]. `r nrow(sona_replaced) - no_missing_sona` participants included less than 5% missing data (i.e., 3 items or less), and these data points amounted to less than 2% of data missing per question. These items were replaced with a random imputation from the *mice* library [CITE MICE]. 

The data were then screen for attentiveness and accuracy using the framework from CITE BUCHANAN to find low quality responses. The survey items were split into three pages, and each page was screened separately using click counts, an attention check item (i.e., please answer strongly agree for this item), and number of characters on a page for reading speed. The total number of indicators for low quality data was summed for each participant across pages (e.g., 3 indicators by 3 pages = 9 total possible items), and *n* = `r nrow(sona_replaced)- nrow(sona_good)` participants showed at least 4 indicators across all pages. CITE BUCHANAN suggest a criteria of 2/5 screeners or 40%, and therefore, we used 4 items as the ~40% cut off for inclusion in the data. The final sample size for the undergraduate population was *n* = `r nrow(sona_good)`. We additionally screened for multivariate outliers using Mahalanobis distance across items and found *n* = `r nrow(sona_good) - nrow(sona_noout)` possible outliers, but they are included in the analyses described below. 
...
The Mechanical Turk data were screened in the same manner as the undergraduate sample. The original sample size was *n* = `r nrow(mturk)` participants. *N* = `r nrow(mturk) -  nrow(mturk_replaced)` participants were excluded for missing more than 5% of their data, and *n* = `r nrow(mturk_replaced) - no_missing_turk` participants had 3 or less data points estimated using the *mice* library. The same low quality data screening was performed, and *n* = `r nrow(mturk_replaced) - nrow(mturk_good)` participants were excluded for having 40% or more low quality indicators. Last, *n* =  `r nrow(mturk_good) - nrow(mturk_noout)` participants were flagged as potential outliers but were not included in analyses presented below. 
...
The data was then randomly split into *n* = `r nrow(efaDF)` participants for the exploratory factor analysis, and *n* = `r nrow(cfaDF)` for the confirmatory factor analysis. The EFA data included *n* undergraduate = `r nrow(efaDF %>% filter(where == "Undergraduate"))` participants, and *n* Mechanical Turk = `r nrow(efaDF %>% filter(where != "Undergraduate"))` participants. The CFA data included *n* undergraduate = `r nrow(cfaDF %>% filter(where == "Undergraduate"))` participants, and *n* Mechanical Turk = `r nrow(cfaDF %>% filter(where != "Undergraduate"))` participants. 
...
The `r ncol(efaDF)` items were first tested with a 2-factor fit, and `r two_factor_1["no"]` items were excluded for not loading on any factor or loading on multiple factors with a loading greater than .30. After removing these items, all items loaded cleanly on the two factors. The loading table for this analysis can be found at XX. This same procedure was repeated for the 3-factor fit, and `r three_factor_1["no"]` items were removed from the first round of analysis, and `r three_factor_2["no"]` item was removed from the second round of analysis, resulting in a final set of `r three_factor_3["yes"]` items. The loading table and items can be found at XX. Last, the 5-factor model showed `r five_factor_1["0"] + five_factor_1["2"]` poor items on the first round of analysis, and `r five_factor_2["0"] + five_factor_2["2"]` poor items on the second round of analysis, resulting in a final set of `r five_factor_3` items for the final model. The items and loadings can be found at XX. 
...
We then compared items across all factor models to reduce scale length. Items were selected for the final model if they: 1) theoretically made sense with the other items on that factor, 2) loaded on the theorized 5-factor model (i.e., items were included in the 2-factor model if they also had been included in the 5-factor model), and 3) were not face-value redundant with other items on that factor. We tested these items and found that the 2-factor and 3-factor models continued to show simple structure, but the 5-factor model required the removal of `r five_loadings_1["2"] + five_loadings_2["2"]` more items due to cross loadings across factors. To select the best model, we examined the fit indices for each factor structure. The 5-factor model showed the best fit (RMSEA = `r apa_num(EFA_5factor$RMSEA[1], gt1 = F, digits = 3)`, RMSR = `r apa_num(EFA_5factor$rms, gt1 = F, digits = 3)`, TLI = `r apa_num(EFA_5factor$TLI, gt1 = F, digits = 3)`, CFI = `r apa_num(EFA_5factor$CFI, gt1 = F, digits = 3)`), followed by the 3-factor model (RMSEA = `r apa_num(EFA_3factor$RMSEA[1], gt1 = F, digits = 3)`, RMSR = `r apa_num(EFA_3factor$rms, gt1 = F, digits = 3)`, TLI = `r apa_num(EFA_3factor$TLI, gt1 = F, digits = 3)`, CFI = `r apa_num(EFA_3factor$CFI, gt1 = F, digits = 3)`), and then the 2-factor model (RMSEA = `r apa_num(EFA_2factor$RMSEA[1], gt1 = F, digits = 3)`, RMSR = `r apa_num(EFA_2factor$rms, gt1 = F, digits = 3)`, TLI = `r apa_num(EFA_2factor$TLI, gt1 = F, digits = 3)`, CFI = `r apa_num(EFA_2factor$CFI, gt1 = F, digits = 3)`). The final factor loadings for the 5-factor scale are shown in Table \@ref(tab:table-five-factor), and the other loadings can be found at XX (2-factor) and XX (3-factor).
...
Using the $\Delta$CFI $\le$ .01 rule for degradation in fit, we found that the loadings (*metric*) showed invariance across groups, while a large degradation in fit was found between the loadings and intercepts (*scalar*). We individually removed the equality constraint between groups for each item, and found one item that was different between groups: *I have children or students which take after me.* The intercept for the undergraduate sample on this item was: `r intercepts_diff %>% filter(group == 1) %>% pull(estimate) %>% apa_num()`, *SE* = `r intercepts_diff %>% filter(group == 1) %>% pull(std.error) %>% apa_num()`, while the Mechanical Turk sample showed a higher average item score: `r intercepts_diff %>% filter(group == 2) %>% pull(estimate) %>% apa_num()`, *SE* = `r intercepts_diff %>% filter(group == 2) %>% pull(std.error) %>% apa_num()`. Figure \@ref(fig:visualize-diffs) demonstrates a visualization of those differences with higher intercepts for the Mechanical Turk group. The overall distribution for the biological factor is also displayed in this figure. 
...
Following the partial intercepts model, we found non-invariance when all residuals were constrained to equal. The same steps were applied and 2 non-invariant items were discovered that would allow the model to be partially invariant. First, *I believe in an afterlife* showed larger residuals for the undergraduate population `r variance_diff %>% filter(group == 1) %>% filter(term == "Q4_19 ~~ Q4_19") %>% pull(estimate) %>% apa_num()` (*SE* = `r variance_diff %>% filter(group == 1) %>% filter(term == "Q4_19 ~~ Q4_19") %>% pull(std.error) %>% apa_num()`), while the Mechanical Turk population showed a much smaller amount of variance `r variance_diff %>% filter(group == 2) %>% filter(term == "Q4_19 ~~ Q4_19") %>% pull(estimate) %>% apa_num()` (*SE* = `r variance_diff %>% filter(group == 2) %>% filter(term == "Q4_19 ~~ Q4_19") %>% pull(std.error) %>% apa_num()`). The second item was a similar item, *My spirit or soul will live on after I physically die*, where the undergraduate population showed a much larger variance `r variance_diff %>% filter(group == 1) %>% filter(term == "Q2_17 ~~ Q2_17") %>% pull(estimate) %>% apa_num()` (*SE* = `r variance_diff %>% filter(group == 1) %>% filter(term == "Q2_17 ~~ Q2_17") %>% pull(std.error) %>% apa_num()`) than the Mechanical Turk population `r variance_diff %>% filter(group == 2) %>% filter(term == "Q2_17 ~~ Q2_17") %>% pull(estimate) %>% apa_num()` (*SE* = `r variance_diff %>% filter(group == 2) %>% filter(term == "Q2_17 ~~ Q2_17") %>% pull(std.error) %>% apa_num()`). Figure \@ref(fig:fig-4-residuals) demonstrates the differences in variance and overall latent variable means for these items on the spiritual factor.  
